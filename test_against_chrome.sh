...
cp -r ./ca/ca_certs ./testing_against_browsers

pushd ./testing_against_browsers
sudo docker build . --tag browser_testing:latest
popd
...
sudo docker run --network host browser_testing:latest
